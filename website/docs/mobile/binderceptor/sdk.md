...
提供了三个不同级别的`Binder`日志输出，级别可叠加，由于应用进程`Binder`通信调用及其频繁，因此建议通常只需开启`SIMPLE`级别即可。
...
由于`Binder`通信日志繁多，分析困难，因此我们需要设置过滤条件的接口，比如只需要打印方法参数中包含目标内容的日志。如只打印`Binder`通信调用中传递的参数包含字符串"com.demo"（`Binder`通信字符通常为`uint16_t`宽字符）内容的调用日志。
